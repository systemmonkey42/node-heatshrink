...
static napi_value encoder_close_method(napi_env env, napi_callback_info info) {
  heatshrink_encoder *hse;

  napi_value this;
  napi_get_cb_info(env, info, 0, NULL, &this, NULL);
...
static napi_value encoder_constructor(napi_env env, napi_callback_info info) {
  ...
  if (argType == napi_object) {
    ...
    NAPI_CALL(env, napi_wrap(env, _this, enc, encoder_instance_finalizer, NULL,
                             NULL));
  }
...
static napi_value example_method(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_unwrap(env, this, (void **)&pdata));
  printf("pdata = %s\n", pdata);
  ...
  if (argType == napi_string) {
    ...
    size_t result;
    char buf[128];
    NAPI_CALL(env, napi_get_value_string_latin1(env, argv[0], buf, sizeof(buf),
                                                &result));
...
napi_value create_addon(napi_env env) {
  ...
  napi_property_descriptor encoderProperties[] = {
      {"close", NULL, encoder_close_method, NULL, NULL, NULL, napi_default, NULL},
      {"test", NULL, example_method, NULL, NULL, NULL, napi_default, NULL}};

  napi_property_descriptor decoderProperties[] = {
      {"close", NULL, example_method, NULL, NULL, NULL, napi_default, NULL}};

  NAPI_CALL(env, napi_define_class(env, "Encoder", NAPI_AUTO_LENGTH,
                                   encoder_constructor, NULL, 1,
                                   encoderProperties, &encoderClass));

  NAPI_CALL(env, napi_define_class(env, "Decoder", NAPI_AUTO_LENGTH,
                                   decoder_constructor, NULL, 1,
                                   decoderProperties, &decoderClass));

  NAPI_CALL(env, napi_set_named_property(env, result, "Encoder", encoderClass));

  NAPI_CALL(env, napi_set_named_property(env, result, "Decoder", decoderClass));
